<?php
class TeamBase extends BaseEntity {
	const DB_CONN = 'DB_1';
	...
	public static function LoadById($intId){
		$sql = sprintf("SELECT * FROM %s WHERE idTeam = %s;", self::TABLE_NAME, $intId);
	...
	public static function LoadAll(){
		...
		$result = MLCDBDriver::Query($sql, Team::DB_CONN);
	...
	public function ToXml($blnReclusive = false){
        ...
        $xmlStr .= "<idCaptainUser>";
        $xmlStr .= $this->idCaptainUser;
        $xmlStr .= "</idCaptainUser>";
    ...
    public function GetAnswerArr(){
       return Answer::LoadCollByIdTeam($this->idTeam);
    }
    ...
    public function ParseArray($arrData){
    	foreach($arrData as $strKey => $mixVal){
    		$arrData[strtolower($strKey)] = $mixVal;
             ...
             if(array_key_exists('idteam', $arrData)){
                $this->intIdTeam = $arrData['idteam'];
             }
       ...
       public static function Parse($mixData, $blnReturnId = false){
        	if(is_numeric($mixData)){
        		...
        		return Team::Load($mixData);
        	}elseif(
        		(is_object($mixData)) && 
        		(get_class($mixData) == 'Team')
        	){
        		if(!$blnReturnId){
        			return $mixData;
        		}
        		return $mixData->intIdTeam;
        	}elseif(is_null($mixData)){
        		return null;
        	}else{
        		throw new Exception(__FUNCTION__ . '->Parse - Parameter 1 must be either an intiger or a class type "Team"');
        	}        	
        }
        ...
        public function __toArray(){
            ...
            $arrReturn['_ClassName'] = "Team";
                 ...
                 $arrReturn['idCaptainUser'] = $this->idCaptainUser;
        ...
        public function __get($strName){
	        switch($strName){
	        	...
	        	break;
	   			...
	   			case('IdCaptainUser'): 
	   			case('idCaptainUser'): 
	   				if(array_key_exists('idCaptainUser', $this->arrDBFields)){
	        			return $this->arrDBFields['idCaptainUser'];
	        		}
	        		return null;
	        	break;
	    ...
	    public function __set($strName, $strValue){
	   		...
	   		switch($strName){
	        	...
	        	break;
	   			...
	   			case('IdCaptainUser'): 
	   			case('idCaptainUser'): 
	        		$this->arrDBFields['idCaptainUser'] = $strValue;
	        	break;
